  ...
  return (
    <div className="p-8 space-y-8 max-w-[1600px] mx-auto text-slate-900 dark:text-slate-100">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-display font-black text-slate-900 dark:text-white tracking-tight">License Management</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Generate and manage activation keys for your products.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4">
          <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-lg font-display font-black text-slate-900 dark:text-white mb-6 uppercase tracking-tight">Add License Key</h3>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-5 py-3.5 rounded-2xl text-sm font-medium text-slate-900 dark:text-white focus:border-indigo-600 dark:focus:border-emerald-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-mono placeholder:text-slate-400 dark:placeholder:text-slate-500"
                  />
                  <button 
                    type="button"
                    onClick={() => setNewLicense({ ...newLicense, key: generateLicenseKey() })}
                    className="p-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl hover:bg-indigo-600 dark:hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
              ...
              <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className="px-4 py-3 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-xs font-bold hover:bg-indigo-600 dark:hover:bg-emerald-600 transition-all flex items-center justify-center gap-2"
                  >
                    Generate 10
                  </button>
                  <button 
                    ...
                    className="px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl text-xs font-bold hover:bg-indigo-600 dark:hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-2"
                  >
                    Generate 50
                  </button>
                </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white dark:bg-slate-900 px-8 py-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
            <Search size={18} className="text-slate-400" />
            <input type="text" placeholder="Search licenses..." className="flex-1 bg-transparent border-none outline-none font-medium text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500" />
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {licenses.map((lic) => (
                  <tr key={lic.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-white text-sm">{lic.productName}</td>
                    <td className="px-6 py-4 font-mono text-xs text-indigo-600 dark:text-emerald-400 font-bold">{lic.key}</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                        lic.status === 'available' ? 'bg-green-50 dark:bg-emerald-950/40 text-green-600 dark:text-emerald-400 border border-green-100 dark:border-emerald-800/50' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-slate-700'
                      }`}>
                        {lic.status}
                      </span>
                    </td>
